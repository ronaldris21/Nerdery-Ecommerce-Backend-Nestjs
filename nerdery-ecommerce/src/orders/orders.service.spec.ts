...
import { FrontendConfig, ConfigNames } from 'src/common/config/config.interface';
import {
...
describe('OrdersService', () => {
  ...
  describe('getPaymentApprovedStatus', () => {
    ...
    it('should return isApproved=true for COMPLETED status', async () => {
      const completedOrder = { ...mockOrderOnly, status: OrderStatusEnum.COMPLETED };
    ...
    it('should return isApproved=false for WAITING_PAYMENT status', async () => {
      const waitingPaymentOrder = { ...mockOrderOnly, status: OrderStatusEnum.WAITING_PAYMENT };
